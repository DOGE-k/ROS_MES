...
class RotationSimple:
    def __init__(self):
        ...
        rospy.loginfo("旋转轴节点启动（支持 Arm1/2/3，统一下发话题）")
        ...
        TOPIC_CMD = os.environ.get('ROS_TOPIC_KINEMATICS_ROTATION_CMD_SEQ', '/control/kinematics_rotation_cmd_sequenced')
    ...
    def cmd_callback(self, msg):
        ...
        # 写入数据库
        self._insert_sensor_log(msg.module_id, device_id, target_tick, "下发旋转指令数据")

        rospy.loginfo(f"Arm{arm_idx+1} 旋转指令：{target_delta_deg:+.4f}° → 下发编码：{target_tick}")
