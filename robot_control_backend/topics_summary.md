...
| `/control/adjust_rotation_cmd`       | `RotationCmd`   | 前端/Web            | `control_node` | 前端微调旋转指令（经control\_node时序化后转发） |
| `/control/adjust_swing_cmd`          | `SwingCmd`      | 前端/Web            | `control_node` | 前端微调摆动指令（经control\_node时序化后转发） |
| `/control/adjust_telescopic_cmd`     | `TelescopicCmd` | 前端/Web            | `control_node` | 前端微调伸缩指令（经control\_node时序化后转发） |
...
| `/hardware/sensor_feedback`    | `SensorCmd`     | `feedback_node`, `imu_average_calculator_node` | 前端/Web                               | 压力传感器反馈（含平均值） |
| `/hardware/gyroscope_feedback` | `GyroFeedback`  | `hardware_node`                                | `tuo_luo_yi`                         | 陀螺仪六轴数据反馈     |
| `/hardware/module_cmd`         | `IntCmd`        | `feedback_node`                                | `module_confirme_node`               | 模块控制指令        |
...
| `/arm_fusion_stats`          | `String` | `calculation_node`  | -                   | 算法统计信息（预留的监控接口） |

### 5. 传感器触发话题（Sensor Trigger）

| 话题名称                  | 消息类型     | 发布节点           | 订阅节点                  | 说明              |
| --------------------- | -------- | -------------- | --------------------- | --------------- |
| `/control/sensor_cmd` | `IntCmd` | `control_node` | `sensor_control_node` | 压力传感器触发信号（立即响应） |

### 6. 压力传感器平均值计算话题（Pressure Average）

| 话题名称                               | 消息类型        | 发布节点                                           | 订阅节点                          | 说明              |
| ---------------------------------- | ----------- | ---------------------------------------------- | ----------------------------- | --------------- |
| `/hardware/sensor_raw`             | `SensorCmd` | `feedback_node`                                | `imu_average_calculator_node` | 压力传感器原始数据中转话题   |
| `/control/pressure_sample_trigger` | `IntCmd`    | `sensor_control_node`                          | `imu_average_calculator_node` | 压力传感器5秒数据采集触发信号 |
| `/hardware/sensor_feedback`        | `SensorCmd` | `feedback_node`, `imu_average_calculator_node` | 前端                            | 压力传感器反馈（含平均值）   |
...
### 8. 急停控制话题（Emergency Stop）

| 话题名称                | 消息类型     | 发布节点   | 订阅节点            | 说明                   |
| ------------------- | -------- | ------ | --------------- | -------------------- |
| `/control/softstop` | `IntCmd` | 前端/Web | `softstop_node` | 急停触发指令（device\_id=1） |

### 9. 模块确认话题（Module Confirm）

| 话题名称                              | 消息类型     | 发布节点                    | 订阅节点                   | 说明       |
| --------------------------------- | -------- | ----------------------- | ---------------------- | -------- |
| `/control/module_cmd`             | `IntCmd` | 前端/Web, `feedback_node` | `module_confirme_node` | 模块控制指令   |
| `/control/module_confirm_success` | `IntCmd` | `module_confirme_node`  | 前端                     | 模块确认成功信号 |
...
| 环境变量                                  | 映射话题                                           | 说明                               |
| ------------------------------------- | ---------------------------------------------- | -------------------------------- |
| `ROS_TOPIC_KINEMATICS_ROTATION_CMD`   | `/control/kinematics_rotation_cmd_sequenced`   | **注意**：实际指向 control\_node 的时序化输出 |
| `ROS_TOPIC_KINEMATICS_SWING_CMD`      | `/control/kinematics_swing_cmd_sequenced`      | **注意**：实际指向 control\_node 的时序化输出 |
| `ROS_TOPIC_KINEMATICS_TELESCOPIC_CMD` | `/control/kinematics_telescopic_cmd_sequenced` | **注意**：实际指向 control\_node 的时序化输出 |
...
| `ROS_TOPIC_SENSOR_RAW`                | `/hardware/sensor_raw`                         | 压力传感器原始数据中转话题                    |
| `ROS_TOPIC_SENSOR_AVG`                | `/hardware/sensor_feedback`                    | 压力传感器平均值输出话题                     |
| `ROS_TOPIC_PRESSURE_SAMPLE_TRIGGER`   | `/control/pressure_sample_trigger`             | 压力传感器5秒采集触发话题                    |
| `PRESSURE_SAMPLE_WINDOW`              | 5.0                                            | 数据采集窗口时间（秒）                      |
| `PRESSURE_WINDOW_SIZE`                | 50                                             | 窗口大小（数据点数）                       |
...
| `sensor_control_node.py`         | `pressure_sensor_control_node` | 压力传感器控制（立即响应模式+触发采集） |
| `imu_average_calculator_node.py` | `pressure_average_calculator`  | 压力传感器平均值计算（5秒窗口）     |
| `feedback_node.py`               | `feedback_node`                | 反馈处理与指令转发            |
| `hardware_node.py`               | `hardware_node`                | STM32硬件桥接            |
| `tuo_luo_yi.py`                  | `imu_processor_node`           | 陀螺仪数据解析与角度计算         |
| `softstop_node.py`               | `softstop_node`                | 急停控制（device\_id=1触发） |
